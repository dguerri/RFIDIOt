...
# display options
# standard document
MRZ_FIELD_NAMES = ('Document code', 'Issuing State or Organisation', 'Name', 'Passport Number', 'Check Digit', 'Nationality', 'Date of Birth',
                   'Check Digit', 'Sex', 'Date of Expiry', 'Check Digit', 'Personal Number or other optional elements', 'Check Digit', 'Composite Check Digit')
MRZ_FIELD_LENGTHS = (2, 3, 39, 9, 1, 3, 6, 1, 1, 6, 1, 14, 1, 1)
MRZ_FIELD_DISPLAY = (0, 3, 1, 2, 5, 6, 8, 9, 11)
MRZ_FIELD_KEYS = (44, 57, 65)
# id card
MRZ_FIELD_NAMES_ID = ('Document code', 'Issuing State or Organisation', 'Document Number', 'Check Digit', 'Personal Number or other optional elements',
                      'Check Digit', 'Date of Birth', 'Check Digit', 'Sex', 'Date of Expiry', 'Check Digit', 'Nationality', 'Check Digit', 'Name')
...
def drawfeatures(face, features):
    ...
    for item in features:
        ...
        y = int(item[8:12], 16)
        if Style == 'Arrow':
            face.create_line(0, 0, x, y, fill="Red",
                             arrow="last", width=2, tags="feature")
        if Style == 'Cross':
            face.create_line(x-6, y-6, x+6, y+6, fill="Red",
                             width=2, tags="feature")
            face.create_line(x-6, y+6, x+6, y-6, fill="Red",
                             width=2, tags="feature")
        if Style == 'Target':
            face.create_line(x, y-15, x, y+15, fill="Red",
                             width=3, tags="feature")
            face.create_line(x-15, y, x+15, y, fill="Red",
                             width=3, tags="feature")
    ...
    while pos < len(hexdata):
        ...
        for length in 2, 4:
            if hexdata[pos:pos + length] in DG1_ELEMENTS:
                decoded = True
                print('  tag:', hexdata[pos:pos + length],
                      '('+DG1_ELEMENTS[hexdata[pos:pos+length]]+')')
...
def decode_ef_dg1(data):
    ...
    length = int(passport.ToHex(data[4:5]), 16)
    ...
    while pointer < len(data):
        if data[pointer] == chr(0x80):
            break
        out += '%s' % chr(int(passport.ToHex(data[pointer:pointer+1]), 16))
        pointer += 1
    ...
    while position < end:
        ...
        for length in 4, 2:
            if datahex[position:position + length] in DG2_ELEMENTS:
                ...
                print('  Tag:', tag,
                      '('+DG2_ELEMENTS[tag]+')', '@', position/2, end=' ')
                ...
                if DG2_TYPE[tag] == TEMPLATE:
                    ...
                    if tag == BDB or tag == BDB1:
                        ...
                        print('      Pose Angle: %s' %
                              datahex[position:position + 6])
                        position += 6
                        print('      Pose Angle Uncertainty: %s' %
                              datahex[position:position + 6])
                        position += 6
                        while features > 0:
                            print('      Feature block: %s' %
                    ...
                    position += asn1fieldlength(datahex[position:])
                    print('     data:',
                          datahex[position:position + fieldlength * 2])
                    if tag == '02':
                        instances = int(
                            datahex[position:position + fieldlength * 2], 16)
                    position += fieldlength * 2
        if not decoded:
            print('Unrecognised element @', position/2,
        ...
        for length in 4, 2:
            if datahex[position:position + length] in DG7_ELEMENTS:
                ...
                if tag == '67':
                    ...
                    img.flush()
                    img.close()
                    print('     JPEG image stored in %sEF_DG7.%s' %
                          (tempfiles, Filetype))
                    Display_DG7 = True
                    position += fieldlength * 2
        if not decoded:
...
def jmrtd_personalise(documentnumber, dob, expiry):
    ...
    p1 = '00'
    p2 = '62'
    data = '621B04' + "%02x" % len(documentnumber) + passport.ToHex(documentnumber) + '04' + "%02x" % len(
        dob) + passport.ToHex(dob) + '04' + "%02X" % len(expiry) + passport.ToHex(expiry)
    lc = "%02X" % (len(data) / 2)
    if passport.send_apdu('', '', '', '', '', ins, p1, p2, lc, data, ''):
        return
    if passport.errorcode == '6D00':
        ...
        cla = '10'
        ins = 'VONJEEK_SET_MRZ'
        data = passport.ToHex(documentnumber) + passport.ToHex(calculate_check_digit(documentnumber)) + passport.ToHex(
            dob) + passport.ToHex(calculate_check_digit(dob)) + passport.ToHex(expiry) + passport.ToHex(calculate_check_digit(expiry))
        lc = "%02X" % (len(data) / 2)
        if passport.send_apdu('', '', '', '', cla, ins, p1, p2, lc, data, ''):
...
def help():
    ...
    print('Usage:')
    print(
        '\t' + sys.argv[0] + ' [OPTIONS] <MRZ (Lower)|PLAIN|CHECK|[PATH]> [WRITE|WRITELOCK|SLOWBRUTE]')
    print()
...
if not TEST and not FILES and MRZ:
    ...
    if len(key) == 21:
        key = key[0:9] + 'XXXX' + key[9:15] + \
            'XX' + key[15:21] + 'XXXXXXXXXXXXXXXXX'
    elif len(key) == 90:
...
if not FILES and not TEST:
    ...
    print('Device supports %s Byte transfers' %
          passport.ISO_FRAMESIZE[passport.framesize])
    print()
...
if not FILES and BAC:
    ...
    while iterations:
        ...
        passport.MRPcompsoitecd = calculate_check_digit(passport.MRPnumber + passport.MRPnumbercd + passport.MRPdob +
                                                        passport.MRPdobcd + passport.MRPexpiry + passport.MRPexpirycd + passport.MRPoptional + passport.MRPoptionalcd)
        ...
        if not TEST:
            ...
            print('Corrected MRZ: ' + passport.MRPnumber + passport.MRPnumbercd + passport.MRPnationality + passport.MRPdob + passport.MRPdobcd +
                  passport.MRPsex + passport.MRPexpiry + passport.MRPexpirycd + passport.MRPoptional + passport.MRPoptionalcd+passport.MRPcompsoitecd)
        print('Key MRZ Info (kmrz): ' + kmrz)
        print()
        kseedhash = SHA.new(kmrz.encode('utf-8'))
        kseed = kseedhash.digest()[:16]
        if DEBUG:
            print('Kseed (SHA1 hash digest of kmrz): ' +
                  kseedhash.hexdigest()[:32])
...
for tag in eflist:
    ...
    if tag == EF_SOD:
        ...
        if exitstatus != 0:
            exitstatus = os.system(
                "openssl pkcs7 -in %sEF_SOD.TMP -out %sEF_SOD.PEM -inform DER" % (tempfiles, tempfiles))
            exitstatus = os.system(
    ...
    if tag == EF_DG15:
        ...
        if not exitstatus:
            os.system("openssl rsa -in %sEF_DG15.TMP -out %sEF_DG15.PEM -inform DER -pubin" %
                      (tempfiles, tempfiles))
            print('Key stored in %sEF_DG15.PEM' % tempfiles)
            continue
        if exitstatus:
            exitstatus = os.system(
                "openssl ec -in %sEF_DG15.TMP -inform DER -pubin -text -noout" % tempfiles)
        if not exitstatus:
            os.system("openssl ec -in %sEF_DG15.TMP -out %sEF_DG15.PEM -inform DER -pubin" %
                      (tempfiles, tempfiles))
            print('Key stored in %sEF_DG15.PEM' % tempfiles)
...
if Jmrtd:
    ...
    if STRIP_INDEX:
        ...
        while i-2 < total_length-1:
            ...
            if tag == '5C'.decode('hex'):
                ...
                newindex = ''.join([x for x in list(oldindex) if x.encode(
                    'hex') in clearDGs])
    ...
    print("JMRTD/vonJeek 3DES key set to: " + mrz[FieldKeys[0]:FieldKeys[0]+9] +
          mrz[FieldKeys[1]:FieldKeys[1]+6] + mrz[FieldKeys[2]:FieldKeys[2]+6])
if JmrtdLock:
    jmrtd_lock()
...
if not Nogui:
    ...
    if Filetype == "JP2":
        ...
        exitstatus = os.system("convert %sJP2 %sJPG" %
                               (tempfiles+'EF_DG2.', tempfiles+'EF_DG2.'))
        print("      (converted %sJP2 to %sJPG for display)" %
              (tempfiles+'EF_DG2.', tempfiles+'EF_DG2.'))
        if exitstatus:
            print(
                'Could not convert JPEG 2000 image (%d) - please install ImageMagick' % exitstatus)
            os._exit(True)
        elif Display_DG7:
            os.system("convert %sJP2 %sJPG" %
    ...
    featurebutton = Checkbutton(frame, text="Show Features", command=do_command(
        drawfeatures, canvas, dg2_features))
    featurebutton.grid(row=1, column=0, sticky=W, rowspan=1)
    featurestyle = Radiobutton(frame, text="Arrow", command=do_command(
        changestyle, "Arrow", canvas, dg2_features))
    featurestyle.grid(row=1, column=0, rowspan=1)
    featurestyle.select()
    featurestyle2 = Radiobutton(frame, text="Cross", command=do_command(
        changestyle, "Cross", canvas, dg2_features))
    featurestyle2.grid(row=2, column=0, rowspan=1)
    featurestyle2.deselect()
    featurestyle3 = Radiobutton(frame, text="Circle ", command=do_command(
        changestyle, "Circle", canvas, dg2_features))
    featurestyle3.grid(row=1, column=0, rowspan=1, sticky=E)
    featurestyle3.deselect()
    featurestyle4 = Radiobutton(frame, text="Target", command=do_command(
        changestyle, "Target", canvas, dg2_features))
    ...
    for item in Fields:
        ...
        if FieldNames[item] == "Issuing State or Organisation" or FieldNames[item] == "Nationality":
            Label(frame, text=mrzspaces(mrz[mrzoffset:mrzoffset + FieldLengths[item]], ' ') + '  ' + passport.ISO3166CountryCodesAlpha[mrzspaces(
                mrz[mrzoffset:mrzoffset + FieldLengths[item]], '')], font=font).grid(row=row, sticky=W, column=1)
        else:
            Label(frame, text=mrzspaces(
                mrz[mrzoffset:mrzoffset + FieldLengths[item]], ' '), font=font).grid(row=row, sticky=W, column=1)
        row += 1
    if len(mrz) == 90:
        # Passport cards have a three-line MRZ
        Label(frame, text=mrz[:30], font=fonta,
              justify='center').grid(row=row, columnspan=4)
        row += 1
        Label(frame, text=mrz[30:60], font=fonta,
              justify='center').grid(row=row, columnspan=4)
        row += 1
        Label(frame, text=mrz[60:], font=fonta,
              justify='center').grid(row=row, columnspan=4)
        row += 1
    else:
        Label(frame, text='  ' + mrz[:len(mrz) // 2], font=fonta,
              justify='left').grid(row=row, sticky=W, columnspan=4)
        row += 1
        Label(frame, text='  ' + mrz[len(mrz) // 2:], font=fonta,
              justify='left').grid(row=row, sticky=W, columnspan=4)
        row += 1
