...
class NFC(object):
    ...
    def initlibnfc(self):
        ...
        self.libnfc.nfc_initiator_list_passive_targets.argtypes = [
            ctypes.c_void_p, ctypes.Structure, ctypes.c_void_p, ctypes.c_size_t]
        self.libnfc.nfc_initiator_transceive_bytes.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_uint32]
        self.libnfc.nfc_initiator_target_is_present.argtypes = [
            ctypes.c_void_p, ctypes.Structure]
    ...
    def listreaders(self, target):
        ...
        print('LibNFC ver', self.libnfc.nfc_version(),
              'devices (%d):' % nfc_num_devices)
        if nfc_num_devices == 0:
            print('\t', 'no supported devices!')
            return
        for i in range(nfc_num_devices):
            if devices[i]:
                self.log.debug("nfc_open: %s" % ctypes.cast(
                    devices[i].connstring, ctypes.c_char_p).value)
                dev = self.libnfc.nfc_open(
                    self.context, ctypes.byref(devices[i]))
    ...
    def configure(self, nfcreader):
        if rfidiotglobals.Debug:
            ...
            self.log.debug("Connecting to NFC reader number: %s" %
                           repr(nfcreader))  # nfcreader may be none
        if nfcreader != None:
        ...
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_ACTIVATE_FIELD, False)

        # Let the reader only try once to find a tag
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_INFINITE_SELECT, False)
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_HANDLE_CRC, True)
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_HANDLE_PARITY, True)
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_ACCEPT_INVALID_FRAMES, True)
        # Enable field so more power consuming cards can power themselves up
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_ACTIVATE_FIELD, True)
    ...
    def configMifare(self):
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_AUTO_ISO14443_4, False)
        self.libnfc.nfc_device_set_property_bool(
            self.device, NP_EASY_FRAMING, True)
    ...
    def sendAPDU(self, apdu, timeout=None):
        ...
        if rfidiotglobals.Debug:
            self.log.debug("Sending %d byte APDU: %s" %
                           (len(tx), "".join(["%02x" % x for x in tx])))
        rxlen = self.libnfc.nfc_initiator_transceive_bytes(self.device, ctypes.byref(tx), ctypes.c_size_t(
            len(tx)), ctypes.byref(rx), ctypes.c_size_t(len(rx)), int(timeout * 1000) if timeout is not None else -1)
        if rfidiotglobals.Debug:
            self.log.debug('APDU rxlen = ' + str(rxlen))
        if rxlen < 0:
            self.libnfc.nfc_perror(
                self.device, "nfc_initiator_transceive_bytes")
            if rfidiotglobals.Debug:
