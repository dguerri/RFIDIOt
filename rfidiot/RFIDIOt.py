...
class rfidiot:
        ...
        if not self.NoInit:
            if self.readertype == self.READER_PCSC:
                ...
                if readernum >= len(self.pcsc):
                    print('There is no such reader #%i, PCSC sees only %i reader(s)' % (
                ...
                try:
                    ...
                    print('Could not create connection to %s' %
                          self.readername)
                    os._exit(True)
                ...
                try:
                    ...
                    if hresult != 0:
                        raise error('Failed to establish context: ' +
                                    smartcard.scard.SCardGetErrorMessage(hresult))
                    hresult, readers = smartcard.scard.SCardListReaders(
                        hcontext, [])
                    readerstates = [
    ...
    DES_PAD = [chr(0x80), chr(0), chr(0), chr(0),
    ...
    def info(self, caller):
        ...
        if not self.NoInit:
            ...
            if self.readertype == self.READER_FROSCH:
                print('Frosch ' + self.ToBinary(self.data[:16]) +
                      ' / ' + self.ToBinary(self.data[16:32]), end=' ')
        ...
        if self.readertype == self.READER_LIBNFC:
            try:
                ...
                if cardtype == 'A':
                    ...
                    if self.DEBUG:
                        print('Error: Unknown card type specified: %s' %
                              cardtype)
                    return False
            except ValueError:
    ...
    def acs_send_apdu(self, apdu):
        ...
        if myapdu[0] == 'd4':
            ...
            if myapdu[0] == 'ff' or myapdu[0] == '80':
                ...
                lc = '%02x' % (
                    len(myapdu) + len(self.PCSC_APDU['ACS_14443_A']))
                apduout = self.HexArrayToList(
                    self.PCSC_APDU['ACS_DIRECT_TRANSMIT']+[lc]+self.PCSC_APDU['ACS_14443_A']+myapdu)
    ...
    def acs_mifare_login(self, block, key, keytype):
        ...
        if self.tagtype == self.ACS_TAG_MIFARE_1K or self.tagtype == self.ACS_TAG_MIFARE_4K:
            status = self.acs_send_apdu(
                self.PCSC_APDU['ACS_MIFARE_LOGIN']+[keytype]+[loginblock]+[key]+[self.uid])
        else:
    ...
    def acs_read_block(self, block):
        ...
        if self.tagtype == self.ACS_TAG_MIFARE_ULTRA or self.tagtype == self.ACS_TAG_MIFARE_1K or self.tagtype == self.ACS_TAG_MIFARE_4K:
            status = self.acs_send_apdu(
                self.PCSC_APDU['ACS_READ_MIFARE']+[readblock])
            read = True
        if read:
    ...
    def gp_external_authenticate(self, host_cryptogram, mac_key):
        ...
        mac = self.ToHex(self.DESMAC(self.ToBinary(
            cla+'82'+p1+p2+lc+data), mac_key, ''))
        data += mac
    ...
    def iso_7816_fail(self, code):
        ...
        if code == self.ACG_FAIL:
            ...
            os._exit(True)
        print("Failed - reason code " + code +
              " (" + self.ISO7816ErrorCodes[code] + ")")
        print()
        os._exit(True)
...
#		return ret[4:int(le,16) * 2 + 4]
        ...
        if self.readertype == self.READER_PCSC:
            ...
            if key not in self.PCSC_Keys:
                ...
                if self.readersubtype == self.READER_OMNIKEY:
                    ...
                    # SCM Mifare keys live at hex 60+
                    keynum = len(self.PCSC_Keys) + 96
                if keytype == 'BB':
                    keynumoffset = 1
                else:
                    keynumoffset = 0
                # p2 - key number
                apdu.append('%02x' % (keynum + keynumoffset))
    ...
    def readblock(self, block):
        ...
        if self.readertype == self.READER_ACG:
            ...
            while count * 2 < len(self.data):
                self.binary += chr(int(self.data[count *
                                   2:(count * 2) + 2], 16))
                count += 1
            return True
        if self.readertype == self.READER_LIBNFC:
    ...
    def MIFAREkb(self, data):
        ...
        self.MIFAREblock0AC = str(
            self.MIFAREC1 & 0x01) + str(self.MIFAREC2 & 0x01) + str(self.MIFAREC3 & 0x01)
        self.MIFAREblock1AC = str((self.MIFAREC1 & 0x02) >> 1) + str(
            (self.MIFAREC2 & 0x02) >> 1) + str((self.MIFAREC3 & 0x02) >> 1)
        self.MIFAREblock2AC = str((self.MIFAREC1 & 0x04) >> 2) + str(
            (self.MIFAREC2 & 0x04) >> 2) + str((self.MIFAREC3 & 0x04) >> 2)
        self.MIFAREblock3AC = str((self.MIFAREC1 & 0x08) >> 3) + str(
            (self.MIFAREC2 & 0x08) >> 3) + str((self.MIFAREC3 & 0x08) >> 3)
    ...
    def TRANSITID(self, data):
        ...
        if(lrc != int(data[48:56], 2)):
            print('LRC mismatch: %02X should be %02X!' %
                  (int(data[48:56], 2), lrc))
            return 0
    ...
    def PCSCPrintATR(self, data):
        ...
        print('            %s  %d bytes historical data follow' %
              (data[3], histlen))
        if data[4] == '8':
